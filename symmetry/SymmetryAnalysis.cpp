...
std::vector< SurfaceMesh::SurfaceMeshModel* > SymmetryAnalysis::connectedPieces(SurfaceMesh::SurfaceMeshModel * mesh) {
    ...
    foreach( SurfaceMesh::Face f, mesh->faces() )
    {
        ...
        while( !unprocessed.empty() )
        {
            ...
            foreach(auto v, mesh->vertices(curFace))
            {
                foreach(auto fj, mesh->faces(v))
                {
        ...
        foreach(auto v, mesh->vertices())
        {
            piece->add_vertex( points[v] );
        }

        foreach(auto f, pieceFaces.back()){
            std::vector<SurfaceMesh::Vertex> face;
            foreach(auto v, mesh->vertices(f)) face.push_back(v);
            piece->add_face(face);
        }

        foreach(auto v, piece->vertices()) if(piece->is_isolated(v)) piece->remove_vertex(v);
	...
	// Check for concentric parts
	{
		...
		if( isHasConcentric ){
			for(int i = 0; i < N; i++){
                ...
                foreach(auto v, meshes[i]->vertices())
