...
{
	if ( given_normal.dot(this->Normaldirection()) < 0 )
	{
...
std::vector<Monge_via_jet_fitting::Monge_form> Monge_via_jet_fitting::fit_points(const std::vector<Vector_3> & input, 
																				 const std::vector< std::vector<int> > & neighbours, 
																				 size_t d, size_t dprime)
{
	std::vector<Monge_via_jet_fitting::Monge_form> result;

	// Simple check
	size_t min_nb_points = (d + 1) * (d + 2) / 2;

	for(size_t i = 0; i < input.size(); i++)
	{
		// gather points around current point as given with input
		std::vector<Vector_3> point_range;
		for(size_t j = 0; j < neighbours[i].size(); j++) 
			point_range.push_back( input[ neighbours[i][j] ] );

		// This point is isolated?
		if(point_range.size() < min_nb_points){
			result.push_back(Monge_via_jet_fitting::Monge_form());
			continue;
		}

		// Compute local differential quantities at point
		Monge_via_jet_fitting mfit;
		result.push_back( mfit(point_range.begin(), point_range.end(), d, dprime) );
	}
