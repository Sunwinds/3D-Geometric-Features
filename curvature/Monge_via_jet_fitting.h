...
public:
	...
	public:
		...
		const Vector_3 Normaldirection() const { return m_n; }
		Vector_3& Normaldirection() { return m_n; }
...
public:
	...
	Monge_form operator()(InputIterator begin, InputIterator end,  size_t d = 2, size_t dprime = 2);
	const FT inverse_condition_number() const {return inverse_condition_nb;}
	const std::pair<FT, Vector_3> pca_basis(size_t i) const {return m_pca_basis[i];}

	static std::vector<Monge_via_jet_fitting::Monge_form> fit_points(const std::vector<Vector_3> & input, 
		const std::vector< std::vector<int> > & neighbours, size_t d = 2, size_t dprime = 2);
