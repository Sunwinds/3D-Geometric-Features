...
// See permute
  template<class BidirIter, class Function>
    bool
    permute_ (BidirIter first1, BidirIter last1, typename std::iterator_traits<BidirIter>::difference_type d1,
              Function& f)
    {
      using std::swap;
      switch (d1)
...
//    squeeze a little more performance out of it.
  template<class BidirIter, class Function>
    bool
    permute (BidirIter first1, BidirIter last1, typename std::iterator_traits<BidirIter>::difference_type d1,
             Function& f)
    {
      using std::swap;
      switch (d1)
...
template<class BidirIter>
  ...
  {
    return count_each_combination<size_t> (std::distance (first, mid), std::distance (mid, last));
  }
...
template<class BidirIter>
  ...
  {
    return count_each_permutation<size_t> (std::distance (first, mid), std::distance (mid, last));
  }
...
//   permuting [first+1, last).
  template<class Function, class BidirIter>
    ...
    public:
      explicit
      circular_permutation (Function f, D s) :
          f_ (f), s_ (s)
      {
      }
...
template<class BidirIter>
  ...
  {
    return count_each_circular_permutation<size_t> (std::distance (first, mid), std::distance (mid, last));
  }
...
//     call f() for each permutation of [first3, last3).
  template<class Function, class BidirIter>
    ...
    public:
      rev3 (Function f, BidirIter first1, BidirIter last1, D d1, BidirIter first2, BidirIter last2, D d2,
            BidirIter first3, BidirIter last3, D d3) :
          f_ (f), first1_ (first1), last1_ (last1), d1_ (d1), first2_ (first2), last2_ (last2), d2_ (d2), first3_ (
              first3), last3_ (last3), d3_ (d3)
      {
      }
...
template<class BidirIter>
  ...
  {
    return count_each_reversible_permutation<size_t> (std::distance (first, mid), std::distance (mid, last));
  }
...
//   item and reverse-permuting [first+1, last).
  template<class Function, class BidirIter>
    ...
    public:
      explicit
      reverse_circular_permutation (Function f, D s) :
          f_ (f), s_ (s)
      {
      }
...
template<class BidirIter>
  ...
  {
    return count_each_reversible_circular_permutation<size_t> (std::distance (first, mid),
