...
BofSearchManager::BofSearchManager(QString folderPath, bool isCacheImages) : folderPath(folderPath)
{
	...
	if( isCacheImages )
	{
		QStringList files = QDir(folderPath).entryList( QStringList() << "*.png" );
        for(auto imageFilename : files) cachedImages.push_back( QImage(folderPath + "/" + imageFilename) );
	}
}

std::vector<dist_idx_t> BofSearchManager::search(const cv::Mat &image, bool isNormalizeSimiliarity)
{
    MapType data;
    ...
    if( isNormalizeSimiliarity )
    {
        double minVal = results.back().first;
        double maxVal = results.front().first;
        double range = maxVal - minVal;

        for(auto & r : results)
        {
            r.first = (r.first - minVal) / range;
        }
    }
