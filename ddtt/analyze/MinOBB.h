...
/*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*
* gdiam.hpp -
*  Implements an algorithm for computing a diameter,
*
...
#define __MINMAX_DEFINED
	...
	*/
	...
	inline gdiam_real pnt_length( gdiam_point pnt ) 
	{
	...
	public:
		...
		gdiam_real length(int i){
			if(i == 0) return high_1 - low_1;
			if(i == 1) return high_2 - low_2;
			return high_3 - low_3;
		}
		...
		void init( const gdiam_point _dir_1,
			...
			const gdiam_point _dir_3 ) {
                ...
                /*if ( ( ! (fabs( pnt_dot_prod( dir_1, dir_2 ) ) < ZERO_EPSILON ) )
					|| ( ! (fabs( pnt_dot_prod( dir_1, dir_3 ) ) < ZERO_EPSILON ) )
					|| ( ! (fabs( pnt_dot_prod( dir_2, dir_3 ) ) < ZERO_EPSILON ) ) ) {
                        ...
                        ////fflush( stdout );
                        ////fflush( stderr );
						assert( fabs( pnt_dot_prod( dir_1, dir_2 ) ) < ZERO_EPSILON );
						assert( fabs( pnt_dot_prod( dir_1, dir_3 ) ) < ZERO_EPSILON );
						assert( fabs( pnt_dot_prod( dir_2, dir_3 ) ) < ZERO_EPSILON );
                }*/
	...
	*  handle really obnoxious inputs well (i.e., points with equal
	*  coordinates, etc.
	\*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*/
...
#define UMD_SIZE  5
		...
		GBBox * p_ap_bbox )
	{
		...
		if ( ( pnt_length( dir_2 ) < ZERO_EPSILON )
			&& ( pnt_length( dir_3 ) < ZERO_EPSILON ) )
			gdiam_generate_orthonormal_base( dir, dir_2, dir_3 );

		if ( ( pnt_length( dir ) == 0.0 )
			|| ( pnt_length( dir_2 ) < ZERO_EPSILON )
			|| ( pnt_length( dir_3 ) < ZERO_EPSILON ) ) {
				gdiam_generate_orthonormal_base( dir, dir_2, dir_3 );
				pnt_dump( dir );
				pnt_dump( dir_2 );
	...
	/*-----------------------------------------------------------------------
	* Code for computing best bounding box in a given direction
	\*-----------------------------------------------------------------------*/
	...
	public:
		...
		bool equal_real( const point2d & pnt ) const {
			return ( ( fabs( x - pnt.x ) < ZERO_EPSILON )
				&& ( fabs( y - pnt.y ) < ZERO_EPSILON ) );
		}
	};
	...
	public:
		...
		bool operator()(const point2d_ptr & lhs, const point2d_ptr & rhs ) 
		{
			double t = cross(*lhs, base, *rhs);
			if (t != 0)
				return t > 0;
			return dist2(*lhs, base) < dist2(*rhs, base);
		}
		...
		inline double cross(const point2d &a, const point2d &o, const point2d &b)
		{
		...
		gdiam_point pnt3 )
	{
		...
		if ( ( pnt_length( pnt1 ) < ZERO_EPSILON )
			&& ( pnt_length( pnt2 ) < ZERO_EPSILON )
			&& ( pnt_length( pnt3 ) < ZERO_EPSILON ) ){
				assert( false );
		}
		...
		if ( ( pnt_length( pnt2 ) < ZERO_EPSILON )
			&& ( pnt_length( pnt3 ) < ZERO_EPSILON ) ) {
		...
		if ( pnt_length( pnt3 ) < ZERO_EPSILON ) {
	...
	public:
		...
		void compute() {
            ...
            /*
			if ( ( ! (fabs( pnt_dot_prod( base_proj, out_1 ) ) < ZERO_EPSILON ) )
				|| ( ! (fabs( pnt_dot_prod( base_proj, out_2 ) ) < ZERO_EPSILON ) )
				|| ( ! (fabs( pnt_dot_prod( out_1, out_2 ) ) < ZERO_EPSILON ) ) ) {
                    //printf( "should be all close to zero: %g, %g, %g\n",
                    ...
                    ////fflush( stdout );
                    ////fflush( stderr );
					assert( fabs( pnt_dot_prod( base_proj, out_1 ) ) < ZERO_EPSILON );
					assert( fabs( pnt_dot_prod( base_proj, out_2 ) ) < ZERO_EPSILON );
					assert( fabs( pnt_dot_prod( out_1, out_2 ) ) < ZERO_EPSILON );
            }*/
		...
		gdiam_bbox bb_out )
	{
		...
		for ( int ind = 0; ind < times; ind++ ) {
			gdiam_point curDir = bb_out.get_dir( ind % 3 );
			double plength = pnt_length( curDir );
		...
		gdiam_real eps )
	{
		...
		bb2 = gdiam_mvbb_optimize( start, size, bb);
		...
		int z_coef )
	{
		...
		bb_new = pps.get_bbox();
		bb_new = gdiam_mvbb_optimize( start, size, bb_new );
		...
		int grid_size )
	{
		...
		bb_out = bb = gdiam_mvbb_optimize( start, size, bb_out );
		...
		bb_out = gdiam_mvbb_optimize( start, size, bb_out );
		...
		int sample_size )
	{
		...
		grid_size = (int)std::sqrt( double(sample_size / 2) );
	...
	struct OBB
	{
		...
		struct OBBOX{
			...
			OBBOX(gdiam_bbox & box){
				HalfSize[0] = 0.5 * ( box.high_1 - box.low_1 );
				HalfSize[1] = 0.5 * ( box.high_2 - box.low_2 );
				HalfSize[2] = 0.5 * ( box.high_3 - box.low_3 );

				for(int i = 0; i < 3; i++){
					Axis[0][i] = box.dir_1[i];
					Axis[1][i] = box.dir_2[i];
					Axis[2][i] = box.dir_3[i];
				}

				std::vector<Vector3> c(2);
				box.get_vertex( 0, 0, 0, &c[0][0], &c[0][1], &c[0][2] );
				box.get_vertex( 1, 1, 1, &c[1][0], &c[1][1], &c[1][2] );
				Center = (c[0] + c[1]) / 2.0;
			}
		};
		...
		OBB(std::vector<Vector3> all_points, bool isAddJitter = false)
		{
			...
			gdiam_real * points = (gdiam_point)malloc( sizeof( gdiam_point_t ) * num );
			for ( int i = 0; i < num; i++ ) 
			{
				points[ i * 3 + 0 ] = all_points[i][0];
				points[ i * 3 + 1 ] = all_points[i][1];
				points[ i * 3 + 2 ] = all_points[i][2];
			}
            ...
            bb = gdiam_approx_mvbb_grid_sample( pnt_arr, num, 8, 100 );
		...
		gdiam_point minimum() { return bb.minimum; } 
		gdiam_point maximum() { return bb.maximum; }

		template <typename Vector3>
		std::vector<Vector3> corners(){
			std::vector<Vector3> c(8, Vector3(0,0,0));
			bb.get_vertex( 0, 0, 0,		&c[0][0], &c[0][1], &c[0][2] );
			bb.get_vertex( 0, 0, 1,		&c[1][0], &c[1][1], &c[1][2] );
			bb.get_vertex( 0, 1, 0,		&c[2][0], &c[2][1], &c[2][2] );
			bb.get_vertex( 0, 1, 1,		&c[3][0], &c[3][1], &c[3][2] );
			bb.get_vertex( 1, 0, 0,		&c[4][0], &c[4][1], &c[4][2] );
			bb.get_vertex( 1, 0, 1,		&c[5][0], &c[5][1], &c[5][2] );
			bb.get_vertex( 1, 1, 0,		&c[6][0], &c[6][1], &c[6][2] );
			bb.get_vertex( 1, 1, 1,		&c[7][0], &c[7][1], &c[7][2] );
			return c;
		}

		template <typename Vector3>
		std::vector< std::vector<Vector3> > faces(){
			std::vector<Vector3> c = corners<Vector3>();
			std::vector< std::vector<Vector3> > f(12, std::vector<Vector3>(3));
			f[0][0] = c[2]; f[0][1] = c[1]; f[0][2] = c[0];
			f[1][0] = c[3]; f[1][1] = c[1]; f[1][2] = c[2];
			f[2][0] = c[1]; f[2][1] = c[3]; f[2][2] = c[7];
			f[3][0] = c[1]; f[3][1] = c[7]; f[3][2] = c[5];
			f[4][0] = c[4]; f[4][1] = c[5]; f[4][2] = c[7];
			f[5][0] = c[4]; f[5][1] = c[7]; f[5][2] = c[6];
			f[6][0] = c[0]; f[6][1] = c[1]; f[6][2] = c[4];
			f[7][0] = c[1]; f[7][1] = c[5]; f[7][2] = c[4];
			f[8][0] = c[2]; f[8][1] = c[6]; f[8][2] = c[7];
			f[9][0] = c[2]; f[9][1] = c[7]; f[9][2] = c[3];
			f[10][0] = c[0]; f[10][1] = c[6]; f[10][2] = c[2];
			f[11][0] = c[0]; f[11][1] = c[4]; f[11][2] = c[6];
			return f;
		}

		template <typename Vector3>
		static bool testObbObb(const OBBOX<Vector3> &BoxA, const OBBOX<Vector3> &BoxB)
		{
			size_t i, j;
			double ra, rb;
			...
			t = Vector3(t.dot(BoxA.Axis[0]), t.dot(BoxA.Axis[1]), t.dot(BoxA.Axis[2]));
			...
			{
				ra = BoxA.HalfSize[i];
				rb = BoxB.HalfSize[0] * AbsR[i][0] + BoxB.HalfSize[1] * AbsR[i][1] + BoxB.HalfSize[2] * AbsR[i][2];
			...
			for (i = 0; i < 3; ++i)
			{
				ra = BoxA.HalfSize[0] * AbsR[0][i] + BoxA.HalfSize[1] * AbsR[1][i] + BoxA.HalfSize[2] * AbsR[2][i];
				rb = BoxB.HalfSize[i];
				if (std::abs(t[0] * R[0][i] + t[1] * R[1][i] + t[2] * R[2][i]) > ra + rb) return false;
			}

			// Test axis L = A0 x B0
			ra = BoxA.HalfSize[1] * AbsR[2][0] + BoxA.HalfSize[2] * AbsR[1][0];
			rb = BoxB.HalfSize[1] * AbsR[0][2] + BoxB.HalfSize[2] * AbsR[0][1];
		...
		std::vector<double> rotation()
		{
			...
			pnt_normalize( bb.dir_1 );
			pnt_normalize( bb.dir_2 );
			pnt_normalize( bb.dir_3 );
			...
			double m00 = bb.dir_1[0], m01 = bb.dir_2[0], m02 = bb.dir_3[0];
			double m10 = bb.dir_1[1], m11 = bb.dir_2[1], m12 = bb.dir_3[1];
			double m20 = bb.dir_1[2], m21 = bb.dir_2[2], m22 = bb.dir_3[2];

			/// Taken from jMonkey engine:
			...
			if (t >= 0) { // |w| >= .5
				...
				double s = std::sqrt(1.0 + m00 - m11 - m22); // |s|>=1
				x = s * 0.5; // |x| >= .5
				s = 0.5 / s;
				y = (m10 + m01) * s;
				z = (m02 + m20) * s;
				...
				double s = std::sqrt(1.0 + m11 - m00 - m22); // |s|>=1
				y = s * 0.5; // |y| >= .5
				s = 0.5 / s;
				x = (m10 + m01) * s;
				z = (m21 + m12) * s;
				...
				double s = std::sqrt(1.0 + m22 - m00 - m11); // |s|>=1
				z = s * 0.5; // |z| >= .5
				s = 0.5 / s;
				x = (m02 + m20) * s;
				y = (m21 + m12) * s;
